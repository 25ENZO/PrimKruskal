...
//pour vérifier si le graphe est directe
...
//crée un pointeur vers la matrice de cout et d'arrete
int** matriceGeneral;
...
//compter le numéro de lignes pour l'allorcation de Ligne du fichier
...
string parse(string argFile, int k)
{
	...
	string Ligne[4];// Crée le tableau des lignes (4 emplacement)
	string tmp;// valeur temporellle qui va etre renvoyé et correspond a la ligne k
...
{
	int s1 = 0;//int numéro sommet1
	int s2 = 0;//int numéro sommet1
	...
	if (myfile.is_open()) //Si le  fichier est ouvert
	{
		...
		{                       
			...
			for(int i = 0; i <= taille-3; ++i)//boucle parcours données
			{
				getline (myfile,line);
...
{
	int nbNodes = getNbNodes(argFile);
	int min = INT_MAX, min_index;//initialise la valeur min à INT_MAX
	for (int i = 0; i < nbNodes; i++)
	{
...
//crée un fichier outputFile dans lequel on met l'arbre minimal ligne par ligne
...
int prim(string argFile)
{
	...
	for (int i = 0; i < nbNodes; i++)
	{
		key[i] = INT_MAX, mstSet[i] = false;//initialise la valeur min à INT_MAX
	}      
	            
	key[0] = 0; // L'algorithme commence à partir du sommet 0
	parent[0] = -1; // initialise le tableau parent à -1
